...
interface SharePanelProps {
  ...
  /** When true, omit outer card border (e.g. inside a tab panel). */
  embedded?: boolean;
}
...
export default function SharePanel({
  ...
  return (
    <div
      className={cn(
        'space-y-3',
        embedded ? 'p-0.5' : 'rounded-[var(--radius-lg)] border border-white/10 bg-white/5 p-3'
      )}
    >
        ...
        >
          <span
            className={cn(
              ...
              shareActive ? 'translate-x-[18px]' : 'translate-x-[3px]'
            )}
          />
        </button>
      </div>
        ...
        <>
          ...
          <div className="flex items-center gap-3">
            {shareQr ? (
              ...
              <div className="w-20 h-20 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-[10px] text-white/40 shrink-0">
            ...
            <div className="flex-1 min-w-0 space-y-1.5">
              {shareUrl && (
                <div>
                  <p className="text-xs text-white/55">{t('common.sttShareUrlLabel')}</p>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={onStopShare}
              disabled={busy}
              className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 text-xs font-bold transition-colors disabled:opacity-40"
            >
      ...
      {!shareActive && (
        <button
          ...
          disabled={busy}
          className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-dashed border-white/20 px-4 py-1.5 text-xs font-semibold text-white/60 hover:text-white hover:border-white/40 hover:bg-white/5 transition-colors disabled:opacity-40"
        >
          <Smartphone className="w-3.5 h-3.5" aria-hidden="true" />
