...
export default function Toast() {
  ...
  const isDisplayDisconnected = toastMessage === 'displayDisconnected';

  useEffect(() => {
    if (!toastMessage) return;
    if (!isDisplayDisconnected) playSfx(isUndo ? 'undo' : 'redo');
    const timer = setTimeout(() => setToastMessage(null), 1500);
    return () => clearTimeout(timer);
  }, [toastKey, toastMessage, isDisplayDisconnected, isUndo, setToastMessage]);
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-2 px-5 py-2.5 rounded-xl backdrop-blur border shadow-2xl text-sm font-semibold ${
        ...
        {isDisplayDisconnected ? <MonitorOff className="w-4 h-4" /> : isUndo ? <Undo2 className="w-4 h-4" /> : <Redo2 className="w-4 h-4" />}
